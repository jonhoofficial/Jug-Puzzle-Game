...
public class JugPuzzle extends Observable{
	...
	public boolean getIsPuzzleSolved() {
		...
		this.setChanged();
		this.notifyObservers("puzzle solved is " + (jugs[0].getAmount() == 4 && jugs[1].getAmount() == 4));
	...
	public void move(int from, int to) {
		if(0<=from && from<jugs.length && 0<=to && to<jugs.length){
			...
			// notify observers about the change
			this.setChanged();
			this.notifyObservers("move made from " + from + " to " + to);
		}
	}
	...
	public synchronized void addObserver(Observer o) {
		super.addObserver(o);
		this.setChanged();
		this.notifyObservers("created and observed");
	}
}